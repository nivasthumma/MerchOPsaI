...
export default function Dashboard() {
  ...
  const { live, lastUpdated } = useLiveRefresh(load, 5000);
    ...
    <div className="dash">
      ...
      {r.invariants_broken.length > 0 && (
        <div className="banner danger" role="alert">
          ...
          {r.invariants_broken.join("; ")}. Treat the recovery numbers as unreliable until this is resolved.
        </div>
      )}

      <SectionHead title="Revenue recovery">
        <span className="live-state" aria-live="polite">
          <span className={`live-dot ${live ? "on" : "off"}`} />
          {live ? "Live" : "Paused"}{" · "}
          {lastUpdated || fetchedAt ? <When iso={lastUpdated || fetchedAt!} /> : "waiting"}
        </span>
      </SectionHead>
      ...
      <ol className="ledger-chain" aria-label="Recovery ledger">
        {chain.map((c) => (
          <li key={c.key} data-k={c.key}>
            <div className="ledger-bar"><span style={{ width: `${Math.max(pct(c.minor), 1.5)}%` }} /></div>
            <dl><dt>{c.label}</dt><dd><Money minor={c.minor} /></dd></dl>
      ...
      {r.by_incident.length === 0 ? <Empty>No open incidents.</Empty> : (
        <table className="grid">
          <thead><tr><th>Incident</th><th>Severity</th><th>Status</th><th className="num">At risk</th><th className="num">Recoverable</th><th className="num">Recovered</th></tr></thead>
          <tbody>{r.by_incident.map((i) => (
            <tr key={i.incident_id}>
              <td><Link to={`/incidents/${i.incident_id}`}>{i.title}</Link></td>
              <td data-sev={i.severity}>{i.severity}</td><td>{i.status}</td>
      ...
      {r.by_method.length === 0 ? <Empty>Nothing planned yet.</Empty> : (
        <table className="grid">
          <thead><tr><th>Method</th><th className="num">Candidates</th><th className="num">Recoverable</th><th className="num">Recovered</th></tr></thead>
          <tbody>{r.by_method.map((m) => (
            <tr key={m.method}><td>{m.method}</td><td className="num">{m.candidates}</td><td className="num"><Money minor={m.recoverable_minor} /></td><td className="num"><Money minor={m.recovered_minor} /></td></tr>
          ))}</tbody>
        </table>
      )}
      ...
      <StatStrip items={[
        ["Open", data.incidents.open], ["Resolved", data.incidents.resolved],
        ...Object.entries(data.incidents.by_status).map(([k, v]) => [k.toLowerCase().replace(/_/g, " "), v] as [string, number]),
      ]} />

      <SectionHead title="Agent activity" />
      <StatStrip items={[
        ["Investigations", data.agent_activity.investigations], ["Tool calls", data.agent_activity.tool_calls],
        ["Recommendations", data.agent_activity.recommendations], ["Awaiting approval", data.agent_activity.awaiting_approval],
