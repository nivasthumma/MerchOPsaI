...
[![Tests](https://img.shields.io/badge/tests-838%20passed-brightgreen.svg)](#-measured-results)
...
| [📊 Measured results](#-measured-results) | 838 tests · 187/187 scenarios · 113 mutants defined |
...
| Tokens | Expire, rotate through an overlap window, revocable individually or wholesale; refresh is single-use with replay detection (ADR-0049) | SSO, MFA, per-device sessions, httpOnly cookies |
...
Test suite: **838 passed** (`make test`) across unit, security and integration, in
...
833 passed and 5 skipped. They are the cross-replica tests, and a fake that agrees
...
14. **Authentication is HMAC bearer tokens, not an identity provider.** Since
   ...
   What is missing is the identity provider itself: no SSO, no MFA, no
   per-device sessions (so "sign out my old phone" means signing out
   everywhere), no audience claim, and the token still lives in `localStorage`
...
tests/          unit · security · integration  (838 tests)
