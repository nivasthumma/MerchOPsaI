...
def committed_session(_seeded_schema):
    ...
    out, and so is `event_outbox`. `audit_logs` is the one thing deliberately
    left alone -- it is append-only by trigger, and `task_id` on it is a plain
    column rather than a foreign key precisely so the trail outlives the task it
    describes. The outbox has no such property and no such reason: a stale event
    there is read back by anything querying the stream.
    """
    engine = get_engine()
    ...
    # These tests commit for real -- that is the point of them, since a
    # savepoint cannot exhibit a race between two transactions. Every audited
    # write therefore also mirrors an `event_outbox` row that survives the test
    # (ADR-0033), and `tests/integration/test_event_spine.py` reads that table
    # expecting to see only what it published itself. Snapshotted and restored
    # on the same principle as `refunds` above.
    events_before = {
        r[0] for r in s.execute(text("SELECT id FROM event_outbox")).all()
    }
    try:
        yield s, made
    finally:
        ...
        with Session() as cleanup:
            ...
            for pid, amount in before.items():
                cleanup.execute(
                    ...
                    {"a": amount, "p": pid})
            cleanup.execute(
                text("DELETE FROM event_outbox WHERE NOT (id = ANY(:keep))"),
                {"keep": list(events_before) or [""]})
            cleanup.commit()
