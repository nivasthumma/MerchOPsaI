...
def reset_schema() -> None:
    ...
    # `alembic_version` is not in Base.metadata, so drop_all leaves it behind --
    # a row asserting a revision for a schema that no longer exists. The next
    # `scripts/migrate.py` believes it and upgrades from there, against tables
    # built by create_all, which fails on the first migration that creates one.
    # Throwing the schema away throws its version away with it.
    with eng.begin() as c:
